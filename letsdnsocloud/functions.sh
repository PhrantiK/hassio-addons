...
function grabzoneid() {
  ...
  export ZONEID=$(curl -sX GET "https://api.cloudflare.com/client/v4/zones?per_page=50" \
...
function createarecord() {
  ...
  curl -sX POST "https://api.cloudflare.com/client/v4/zones/$ZONEID/dns_records"\
    ...
    --data '{"type":"A","name":"'$LE_DOMAINS'","content":"'$IP'","proxied":'${CF_PROXIED:-false}'}' -o /dev/null
...
function updateip() {
  ...
  curl -sX PUT "https://api.cloudflare.com/client/v4/zones/$ZONEID/dns_records/$AID"\
    ...
    -H "X-Auth-Key: $CF_APIKEY"\
    -H "Content-Type: application/json"\
    --data '{"type":"A","name":"'$LE_DOMAINS'","content":"'$1'","proxied":'${CF_PROXIED:-false}'}' -o /dev/null
